...
io.on('connection', function newConnection(socket) {
	...
	// Functions for each socket.on are on the bottom of the page.
	...
	/* ---------- Game play   ---------- */
	socket.on('cardPlayed', (data) => cardPlayed(socket));
	socket.on('judgeSelection', (data) => judgeSelection(socket, data));
...
function cardPlayed(socket, data) {
...
function judgeSelection(socket, data) {
